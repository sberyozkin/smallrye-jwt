...
public class JwtVerifier {
    ...
    public JwtContext verify(String token) throws InvalidJWTException, UnresolvableKeyException {
        ...
        try {
            jws = JsonWebSignature.parse(token);
        ...
        try {
            ...
            Key verificationKey = keyResolver.resolveKey(jws);
            ...
            String payload = JwsVerifier.builder(verificationKey).algorithm(alg).build().verify(jws);
            ...
            return new JwtContext(jws.headers().type(), claims);
        } catch (ExpiredJWTException e) {
            // Nimbus signals expiry with ExpiredJWTException.
            throw new TokenExpiredException(e.getMessage());
        } catch (java.text.ParseException | BadJOSEException e) {
            // Nimbus reports malformed claims via java.text.ParseException and a failed standard
            // claims check via BadJOSEException.
            throw new InvalidJWTException(e.getMessage());
        } catch (JwsException e) {
            // The signature could not be verified; keep the cause for debugging.
